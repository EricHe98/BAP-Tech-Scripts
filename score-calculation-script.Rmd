...
event_info_key <- filter(sheets_table, sheet_title == "Events (Responses)") %>%
  pull(sheet_key)
...
new_exemptions_id <- filter(event_info, Name == new_event) %>%
  pull(Exemptions)
...
new_exemptions <- data_frame(NetID = "aat413", Name = "Deloitte Information Session")
...
for (netid in melted_cm$NetID){
  for (type in melted_cm$Type){
