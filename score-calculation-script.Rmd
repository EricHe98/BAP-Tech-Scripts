...
new_event_id <- filter(event_info, Name == new_event) %>%
  pull(Attendees) %>%
  as_id()

# Do not convert to Google Drive ID yet; check if exemptions data is available first.

new_exemptions <- filter(event_info, Name == new_event) %>%
  pull(Exemptions)
...
# Read and load new exemptions data. Return empty data frame if no new exemptions data is available.

# if (is.na(new_exemptions_id) == FALSE){
#  new_exemptions_id %>%
#    as_id %>%
#    drive_download(path = "new_exemptions.xlsx", overwrite = TRUE)
#  new_exemptions <- read_excel("new_exemptions.xlsx") %>%
#    cbind(Name = new_event)
#} else {
#  print(paste("No exemptions file for the", new_event, "event"))
#  new_exemptions <- data_frame(NetID = "", Name = "")
#}
...
#if (nrow(exemptions_cm) != 0){
#  dummy_exemptions_cm %<>% cbind(exempt = TRUE)
#  dummy_exemptions_tech %<>% cbind(exempt = TRUE)
#}
...
# Extract NetIDs from OrgSync output email column
...
if (TRUE %in% (melted_cm$exempt & melted_cm$attended)){
  ...
  paste("Members", problem_members$`First Name`, problem_members$`Last Name`, "attended", problem_members$Name, "which they were exempted from; they have been counted as attendants, please correct CM data sheet if they did not actually attend.")
}
