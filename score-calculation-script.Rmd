...
event_info_key <- filter(sheets_table, sheet_title == "Events") %>%
  pull(sheet_key)

event_info_cm_key <- filter(sheets_table, sheet_title == "event_info_cm") %>%
  pull(sheet_key)
...
point_scores_cm_key <- filter(sheets_table, sheet_title == "point_scores_cm") %>%
  pull(sheet_key)

point_scores_tech_key <- filter(sheets_table, sheet_title == "point_scores_tech") %>%
  pull(sheet_key)

cs_signups_key <- filter(sheets_table, sheet_title == "cs_signups") %>%
  pull(sheet_key)
...
cs_signups <- gs_read_csv(ss = gs_key(cs_signups_key))

# MAKE SURE TO KEEP THIS UPDATED EVERY SEMESTER! MAKE SURE THE NETID FIELD IS LABELED "NetID"!

current_candidates <- read_xlsx("BAP-Fall-Candidates.xlsx")
...
if (new_event_info$Type == "Professional" | new_event_info$Type == "Social"){
...
if (new_event_info$Type == "Professional" | new_event_info$Type == "Social"){
  new_exemptions <- new_event_info %>%
    pull(Exemptions)}
else if (new_event_info$Type == "Community"){
  new_exemptions <- new_event_info %>%
    pull(`CS Attendees [Exempted]`)
}
...
# For community service events, candidates who sign up for the events are the only ones NOT exempt from the event

if (new_event_info$Type == "Community"){
  if (is.na(new_event_info$`CS Attendees [Signed Up]` == TRUE)){
    stop(print(paste("No signup data for community service event", new_event)))
  }
new_cs_signups <- new_event_info$`CS Attendees [Signed Up]` %>%
  str_split(pattern = ",") %>%
  unlist %>%
  str_extract(pattern = "(?<=-).*")

new_cs_signups_df <- data_frame(NetID = new_cs_signups) %>%
  mutate(exempt = NetID %in% new_exemptions) %>%
  cbind(Name = new_event)

cs_signups %<>% rbind(new_cs_signups_df)
}

# If professional or social event, read and load new event data.
...
# Parse NetIDs. For Professional and Social, NetIDs are extracted from OrgSync output email column
# For Community, NetIDs are extracted from CS Attendees column.

if (new_event_info$Type == "Professional" | new_event_info$Type == "Social"){
  netID_vec <- str_extract(new_event_data$Email, pattern = ".*?(?=@)")}
else if (new_event_info$Type == "Community"){
  netID_vec <- new_event_info %>%
    pull(`CS Attendees [Attended]`) %>%
    str_split(pattern = ",") %>%
    unlist %>%
    str_extract(pattern = "(?<=-).*")
}
else stop("New event is not Professional, Social or Community event type.")
...
melted_cm <- melt(member_event_cm_mat, id = "NetID", variable.name = "Name", value.name = "attended") %>%
  ...
  full_join(dummy_exemptions_cm, by = c("NetID", "Name")) %>%
  filter(NetID != "")
melted_tech <- melt(member_event_cm_mat, id = "NetID", variable.name = "Name", value.name = "attended") %>%
  left_join(event_info, by = "Name") %>%
  full_join(dummy_exemptions_tech, by = c("NetID", "Name")) %>%
  filter(NetID != "")
...
melted_cm$exempt[which(is.na(melted_cm$exempt & (melted_cm$Type == "Community")))] <- TRUE
...
# Empty exemptions creates an NA row and column, which must be removed at the end. This is only a problem
# at the beginning of the semester. We use the na.omit() function which is dangerous if there is an error.
...
updated_points_cm <- group_by(melted_cm, NetID, Type) %>%
  ...
  left_join(current_candidates, by = "NetID")
...
current_candidate_names2 <- readLines("candidate_netID_mapping.txt")
write(current_candidate_names2, file = "candidate_netID_mapping.txt")
```
