...
cs_points_key <- filter(sheets_table, sheet_title == "Fall-17-Attendance") %>%
  pull(sheet_key)
...
cs_points <- gs_read_csv(ss = gs_key(cs_points_key), skip = 1) %>%
  select(-`First Name`, -`Preferred Name`, -`Last Name`, -`Stern Email Address`, -`Class`)

# Get total cs points

cs_total_points <- select(cs_points, NetID, Community = `Total Points`)
...
cs_events <- select(cs_points, -`Total Points`, -one_of(cs_empty_event_names))
...
# GHETTO REQUEST BY ALEX TO CHANGE ALL MANDATORY EVENTS TO HAVE PENALTY 0.5 DELETE THIS CODE AT END OF SEMESTER
event_info %<>% mutate(Penalty = replace(event_info$Penalty, event_info$Penalty != 0, 0.5))
...
for (member_row in c(1:nrow(updated_points_cm))){
  ...
  email_message <- paste("Email: ", paste0(member_points$NetID, "@stern.nyu.edu"), 
                         ...
                         "\n\nEvents attended: \n\n   ",
                         str_c(member_events_professional$Name[member_events_professional$Attended == "Attended"], collapse = "\n\n    "),
                         "\n\nEvents not attended: \n\n   ",
                         str_c(member_events_professional$Name[member_events_professional$Attended == "NotAttended"], collapse = "\n\n    "),
                         "\n\nEvents whose penalties you have been exempted from (including CS events you did not sign up for): \n\n   ",
                         str_c(member_events_professional$Name[member_events_professional$Attended == "Exempted"], collapse = "\n\n    "),
